...
const Recommended = () => {
    ...
    const recommend = comic.filter(items => items.type === 'recommended')
    // console.log(recommend);
    return (
        <div>
            <h2 className='text-4xl text-center font-bold my-6'>Here is the section of recommended</h2>

            {
                <div className='grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-5 my-10 ml-12'>
                {
                    recommend.map(item => <div key={item._id} className="card w-60 bg-base-100 shadow-xl">
